...
int main()
{
	// queue<BTNode<char>*> btq;
	// BTNode<char> *bt;
	// BTNode<char> *p_lc_lc_lc=GetBTNode('h');
	// BTNode<char> *p_lc_lc=GetBTNode('d', p_lc_lc_lc);
	// BTNode<char> *p_lc_lr=GetBTNode('e');
	// BTNode<char> *p_lr_lc=GetBTNode('f');
	// BTNode<char> *p_lr_lr=GetBTNode('g');
    // BTNode<char> *p_lc=GetBTNode('b', p_lc_lc, p_lc_lr);
    // BTNode<char> *p_lr=GetBTNode('c', p_lr_lc, p_lr_lr);
    // BTNode<char> *p = GetBTNode('a', p_lc, p_lr);  
	// PrintTree(p); 
	//LevelTraversal(p);
	/*Preorder_nrec(p);
	cout<<endl;
	...
	Gotoxy(20, 2);
	cout<<bt->data;
	string str="abcdefg";//报错
	char ch[]={'a', 'b', 'c', 'd', 'e', 'f', 'g'};
	vector<char> vec={ch, ch+7};
	vector<char> sl={ch, ch+3};
	vector< vector<char> > outl;
	vector<char> pl;
	Power(outl, sl, pl);
	char pre[]="ABDHIEJCFKGM";
	char mid[]="HDIBJEAFKCGM";
	PrintTree(MakeLinked(pre, mid, 12),80);
	PrintSet(outl);
	cout<<Get_BT_Depth(p);
	cout<<endl;
	Hanoi(2,'a','b','c');
	...
	char ch[]={'a', 'b', 'c', 'd', 'e', 'f', 'g'};
	vector<char> vec={ch, ch+7};
	BTNode<char> *t=Preorder_rec_buildBT(vec);
	PrintTree(t);
	cout<<"The depth of binary tree is "<<Get_BT_Depth(t);
    getchar();
    return 0;
}
